...
class SiswaController extends Controller
{
    ...
    public function tambah(): void
    {
        ...
        $db->query("INSERT INTO siswa (user_id, nama, jenis_kelamin, no_hp, alamat, asrama, foto) VALUES (:uid, :nama, :jk, :no_hp, :alamat, :asrama, :foto)");
        $db->bind(':uid',    $userId);
        $db->bind(':nama',   $namaBaru);
        $db->bind(':jk',     $jk);
        $db->bind(':no_hp',  $noHp);
    ...
    public function edit(): void
    {
        ...
        $jkBaru = trim($_POST['jenis_kelamin'] ?? '');
        if (!in_array($jkBaru, ['L', 'P'])) $jkBaru = null;
    ...
    /**
     * POST /siswa/reset_kode_wali
     * Mereset kode akses wali murid menjadi NULL (kosong)
     */
    public function reset_kode_wali(): void
    {
        $this->requireAuth();
        if (!$this->isPost()) {
            $this->redirect('siswa');
        ...
        if ($id > 0) {
            ...
            $db->bind(':user_id', $userId);
            if ($db->single()) {
                $db->query("UPDATE siswa SET kode_akses_wali = NULL WHERE id = :id");
                $db->bind(':id', $id);
                if ($db->execute()) {
                    Flash::set('success', 'Kode Akses Wali Murid berhasil di-reset. Mereka akan diminta membuat kode baru.');
                } else {
                    Flash::set('error', 'Gagal mereset kode akses wali murid.');
                }
            } else {
                Flash::set('error', 'Siswa tidak ditemukan atau bukan dari kelas Anda.');
            }
